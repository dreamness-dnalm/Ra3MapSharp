...
public class WorldInfoAsset: BaseAsset
{
    ...
    {
        return AssetNameConst.WorldInfo;
    }

    protected override void _Parse(BaseContext context)
    {
        Properties = AssetProperties.FromBytes(this.Data, context);
    ...
    public static WorldInfoAsset Default(string defaultTexture, BaseContext context)
    {
        var asset = new WorldInfoAsset();
        
        asset.ApplyBasicInfo(context);

        var terrainTextureStrings = DefaultSupportedTextureDict[defaultTexture];
        
        var dict = new Dictionary<string, object>()
        {
            ["musicZone"] = "MusicPalette:MusicPalette_NotSet",
            ["weather"] = WeatherType.Normal,
            ["terrainTextureStrings"] = terrainTextureStrings,
            ["mapName"] = "",
            ["mapDescription"] = $"by {Environment.UserName}",
            ["compression"] = CompressionType.RefPack,
            ["cameraGroundMinHeight"] = 0f,
            ["cameraGroundMaxHeight"] = 2500f,
            ["cameraMinHeight"] = 40f,
            ["cameraMaxHeight"] = 550f,
            ["isScenarioMultiplayer"] = false
        };
        
        
        asset.Properties = AssetProperties.FromDict(dict, context);
        ObservableUtil.Subscribe(asset.Properties, asset);
        asset.MarkModified();
        return asset;
    }

    public void RegisterSupportedTexture(string name, string tgaFileName, string nrmFileName)
    {
        if (tgaFileName.EndsWith(".tga"))
        {
            tgaFileName = tgaFileName.Replace(".tga", "");
        }
        
        if (nrmFileName.EndsWith(".nrm"))
        {
            nrmFileName = nrmFileName.Replace(".nrm", "");
        }

        string textureDeclare = tgaFileName + ";" + nrmFileName + ";";
        SupportedTextureDict.Put(name, textureDeclare);
    }
    ...
    public static Dictionary<string, string> DefaultSupportedTextureDict = new Dictionary<string, string>
        {
                // Romania
    ...
    public Dictionary<string, string>
        SupportedTextureDict = new Dictionary<string, string>(DefaultSupportedTextureDict);
}